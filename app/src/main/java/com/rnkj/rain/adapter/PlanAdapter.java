...
public class PlanAdapter extends BaseAdapter{
    ...
    public PlanAdapter(Context context, LinkedList<Plan> plans) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if(plans.get(position).getStatus().equalsIgnoreCase(Plan.PLAN_STATUS_RUNNING)){
            viewHolder.view_status.setBackgroundColor(mContext.getResources().getColor(R.color.plan_txt_status_running));
            viewHolder.plan_status.setText(mContext.getResources().getString(R.string.plan_status_running));
            viewHolder.plan_timecost.setText("用时 : " + DateUtils.getMillon(plans.get(position).getTimeCost()));
        }else if(plans.get(position).getStatus().equalsIgnoreCase(Plan.PLAN_STATUS_WAITING)){
            viewHolder.view_status.setBackgroundColor(mContext.getResources().getColor(R.color.plan_txt_status_readying));
            viewHolder.plan_status.setText(mContext.getResources().getString(R.string.plan_status_waiting));
            viewHolder.plan_timecost.setText("倒计时 : " + DateUtils.getCountDownTime(plans.get(position).getStartDate()));
        }
        if(plans.get(position).getProgress() > 0){
            viewHolder.plan_progress.setText(String.format(mContext.getResources().getString(R.string.mac_progress),plans.get(position).getProgress() + "%"));
        }else{
